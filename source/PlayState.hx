...
//Dialogue
import dialogbox.DialogueBox;
import dialogbox.GarcelloDialogueBox;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{		
		    ...
		    {
			    case 'a.g.o.t.i' | 'foolhardy-remix':
			        var path = Paths.luaAsset(PlayState.SONG.song.toLowerCase()  + "/modchart");
			        ...
			        FileSystem.createDirectory(Main.path + "assets/data/a.g.o.t.i");				
			        FileSystem.createDirectory(Main.path + "assets/data/foolhardy-remix");
		...
		case 'genocide':
		{
			...
			/*var prefixShit:String = 'fuckedpc/PNG_Sequence/StageFire', 'curse';
		...
		case 'headache' | 'nerves':
		{
			...
			var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('garStagebg', 'gar'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.7, 0.7);
			bg.active = false;
			add(bg);

			var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('garStage', 'gar'));
		...
		case 'release':
		{
		    ...
		    var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('garStagebgAlt', 'gar'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.7, 0.7);
			bg.active = false;
			add(bg);
			...
			smoker.frames = Paths.getSparrowAtlas('garSmoke', 'gar');
			...
			var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('garStagealt', 'gar'));
			bgAlley.antialiasing = true;
            bgAlley.scrollFactor.set(0.9, 0.9);
			bgAlley.active = false;
			add(bgAlley);

			var corpse:FlxSprite = new FlxSprite(-230, 540).loadGraphic(Paths.image('gardead', 'gar'));
		...
		case 'fading':
		{
			...
			var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('garStagebgRise', 'gar'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.7, 0.7);
			bg.active = false;
			add(bg);

			var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('garStageRise', 'gar'));
			bgAlley.antialiasing = true;
			bgAlley.scrollFactor.set(0.9, 0.9);
			bgAlley.active = false;
			add(bgAlley);
		...
		case 'foolhardy-remix':
		{
			defaultCamZoom = 0.9;
			curStage = 'maze';
			var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('maze/Maze', 'shared'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
			add(bg);
		...
		switch (curStage)
		{
			...
			case 'garAlleyDead':
			    ...
			    smoke.frames = Paths.getSparrowAtlas('garSmoke', 'gar');
		...
		var gardoof:GarcelloDialogueBox = new GarcelloDialogueBox(false, dialogue);
		gardoof.scrollFactor.set();
		gardoof.finishThing = startCountdown;
		...
		if (isStoryMode)
		{
			...
			{
				case 'headache':
					var introText:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('garIntroText', 'gar'));
					introText.setGraphicSize(Std.int(introText.width * 1.5));
					introText.scrollFactor.set();
					camHUD.visible = false;

					add(introText);
					FlxG.sound.playMusic(Paths.music('city_ambience', 'gar'), 0);
				...
				case 'release':		
                    ...
                    FlxG.sound.play(Paths.sound('Wind_Fadeout', 'gar'));
	...
	function garIntro(?dialogueBox:GarcelloDialogueBox):Void
	{
		var black:FlxSprite = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		black.scrollFactor.set();
		add(black);
	...
	function startSong():Void
	{
		...
		if (FlxG.save.data.songPosition)
		{
			...
			songPosBar.createFilledBar(FlxColor.GRAY, FlxColor.fromString('#FF' + dad.iconColor));
			add(songPosBar);
	...
	{	
		...
		if (SONG.song.toLowerCase() == 'guns' || SONG.song.toLowerCase() == 'screenplay' || SONG.song.toLowerCase() == 'parasite' || SONG.song.toLowerCase() == 'a.g.o.t.i')
		{
		...
		if (curSong.toLowerCase() == 'a.g.o.t.i')
	    {
			if (gf.animation.name != 'hairBlow')
		...
		else
		{
			if (isStoryMode)
			{
				...
				else
				{
		            ...
		            {
			            case 'release':
				            FlxG.switchState(new VideoState('assets/videos/Garcello_Dies.webm', new PlayState()));
						default:
