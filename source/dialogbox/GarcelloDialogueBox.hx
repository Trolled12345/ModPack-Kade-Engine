...
class GarcelloDialogueBox extends FlxSpriteGroup
{
	...
	public function new(talkingRight:Bool = true, ?dialogueList:Array<String>)
	{
		...
		switch (PlayState.SONG.song.toLowerCase())
		{
			...
			case 'headache':
				...
				box.frames = Paths.getSparrowAtlas('dialogue/garBox', 'gar');
				box.animation.addByPrefix('normalOpen', 'Spirit Textbox spawn', 24, false);
				box.animation.addByIndices('normal', 'Spirit Textbox spawn', [11], "", 24);
			case 'nerves':
				hasDialog = true;
				FlxG.sound.play(Paths.sound('garWeak', 'gar'));
		...
		portraitLeft.frames = Paths.getSparrowAtlas('dialogue/gardialogue', 'gar');
		...
		portraitRight.frames = Paths.getSparrowAtlas('dialogue/bf_norm', 'gar');
		portraitRight.animation.addByPrefix('enter', 'Boyfriend portrait Enter', 24, false);
		...
		handSelect = new FlxSprite(FlxG.width * 0.9, FlxG.height * 0.9).loadGraphic(Paths.image('dialogue/hand_textbox', 'gar'));
		add(handSelect);
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.ANY #if mobile || justTouched #end && dialogueStarted == true)
		{
			...
			FlxG.sound.play(Paths.sound('Generic_Text', 'gar'), 0.8);
